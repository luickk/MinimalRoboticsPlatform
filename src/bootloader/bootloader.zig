...
export fn bl_main() callconv(.Naked) noreturn {
    ...
    // const kernel_mapping = mmu.Mapping{ .mem_size = board.Info.mem.ram_len, .virt_start_addr = board.Addresses.vaStart, .phys_addr = board.Info.mem.ram_start_addr + board.Info.mem.bl_load_addr, .granule = Granule.Fourk, .flags = null };
